...
# -----------------------------------------------------------------------------
# Bookshelf (top-level, standalone)
#
# A Bookshelf hosts Knowledge Bases (indexed document search) and provisions its
# own managed Azure AI Search behind network hardening. It is NOT a child of the
# workspace -- it hangs off the resource group like the supercomputer.
#
# Region: the Bookshelf MUST be in the same region as the supercomputer nodepool
# that indexes it, or indexing fails with "The nodepool and bookshelf must be in
# the same region." Both use var.location, so they stay aligned.
#
# Subnets: uses the dedicated searchSubnet (delegated, not consumed by the
# workspace) for its search services, and shares the privateEndpointSubnet for
# its data-plane private endpoint (the PE subnet may be shared across Discovery
...
resource "azapi_resource" "bookshelf" {
  ...
  # The workload identity needs its data-plane grant before the Bookshelf binds.
  depends_on = [azurerm_role_assignment.workload_storage_blob_data_contributor]

  # Bookshelf create provisions a managed AI Search + private endpoints; extend
  # past azapi's 30m default so we don't cancel a healthy in-flight provision.
  timeouts {
    create = "60m"
    update = "60m"
    delete = "60m"
