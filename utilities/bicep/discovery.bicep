...
@description('Name of the Azure Container Registry that hosts Saint-Gobain custom Discovery tool images. Globally unique, 5-50 alphanumeric characters.')
@minLength(5)
@maxLength(50)
...
// Azure Container Registry for Saint-Gobain custom Discovery tool images.
// RBAC-only auth (admin user and anonymous pull disabled) so access is governed entirely by role assignments.
...
// Kubelet identity: pull custom tool container images - scoped to this registry only (least privilege).
...
@description('Resource ID of the Azure Container Registry for custom tool images.')
output acrId string = acr.id

@description('Login server of the Azure Container Registry (use for docker push/pull of custom tools).')
