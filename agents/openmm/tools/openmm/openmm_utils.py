...
def add_reporters(simulation: 'openmm.app.Simulation',
    ...
    Checkpoint files are written to OUTPUT_DIR (/output/) by default so they
    survive OOM kills, pod evictions, and cancellation. Trajectory and log
    files are written to the working directory (large files stay local until
    quick_finish copies them).
    ...
    # Default checkpoint to /output/ for OOM/cancellation survival
...
def run_nvt(simulation: 'openmm.app.Simulation',
    ...
    if output_prefix is not None:
        ...
        if checkpoint_file is None:
            checkpoint_file = os.path.join(OUTPUT_DIR, f'{output_prefix}.chk')
        output_state = f'{output_prefix}_state.xml'
    if checkpoint_file is None:
        checkpoint_file = os.path.join(OUTPUT_DIR, 'nvt.chk')
...
def run_npt(simulation: 'openmm.app.Simulation',
    ...
    if output_prefix is not None:
        ...
        output_state = f'{output_prefix}_state.xml'
    if checkpoint_file is None:
        checkpoint_file = os.path.join(OUTPUT_DIR, 'npt.chk')
...
def run_production(simulation: 'openmm.app.Simulation',
    ...
    if output_prefix is not None:
        ...
        output_state = f'{output_prefix}_state.xml'
    if checkpoint_file is None:
        checkpoint_file = os.path.join(OUTPUT_DIR, 'production.chk')
    if trajectory_interval is not None:
        report_interval = trajectory_interval
...
def save_checkpoint(simulation, filename: str = None):
    """Save simulation checkpoint.

    Args:
        simulation: OpenMM Simulation object
        filename: Output checkpoint file path. Default: OUTPUT_DIR/checkpoint.chk
    """
    if filename is None:
        filename = os.path.join(OUTPUT_DIR, 'checkpoint.chk')
