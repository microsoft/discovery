...
This agent operates as a `kind: prompt` agent within Discovery Studio.
    ...
    User Input → OpenMM (LLM) → OpenMM Tool (Container) → Results

- **Model:** Configured via the `{{model}}` parameter at deploy time
- **Tool:** OpenMM container for GPU-accelerated molecular dynamics simulations with MDTraj
...
|---|---|---|
| `openMM` | `tools/openmm/` | Python code environment with OpenMM for GPU-accelerated molecular dynamics simulations. |
