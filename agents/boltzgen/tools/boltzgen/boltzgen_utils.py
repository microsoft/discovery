...
def _json_safe(value: Any) -> Any:
    """Recursively convert numpy/pandas/Path values into JSON-serializable Python types.

    Walks dicts, lists, tuples, and sets so that nested structures (e.g. a dict
    containing a list of DataFrames) are fully sanitized — the previous
    single-level ``default=`` callback in ``save_final_results`` could silently
    drop or stringify deeply nested numpy values.
    """
    ...
    if isinstance(value, pd.DataFrame):
        return [_json_safe(rec) for rec in value.to_dict('records')]
...
def save_final_results(results: Dict, output_files: Dict = None,
    ...
    """
    final_data: Dict[str, Any] = {"status": status, "summary": _json_safe(results)}
    if output_files:
        final_data["output_files"] = _json_safe(output_files)
    if file_descriptions:
        final_data["file_descriptions"] = _json_safe(file_descriptions)
...
TARGET_STRUCTURE_EXTENSIONS = ('.pdb', '.cif', '.mmcif')


def list_target_files(input_dir: str = None) -> List[str]:
    """Return sorted absolute paths of structure files in ``input_dir``.

    Looks for ``*.pdb``, ``*.cif``, and ``*.mmcif``. Use this before invoking
    ``boltzgen check`` or ``run_design_pipeline`` so the agent can fail fast
    with a clear message when the user forgot to upload a target structure.

    Args:
        input_dir: Directory to search. Defaults to the global ``INPUT_DIR``.
    ...
    files: List[str] = []
    for ext in TARGET_STRUCTURE_EXTENSIONS:
        files.extend(sorted(glob.glob(os.path.join(base, f'*{ext}'))))
    return [os.path.abspath(f) for f in files]


def validate_target_file(path: str) -> Dict[str, Any]:
    """Cheap structural sanity check on a PDB / mmCIF file.

    Confirms the file exists, has a structural extension, and contains at least
    one of the ``ATOM`` / ``_atom_site.`` / mmCIF ``data_`` markers. This is an
    O(ms) precheck — it does NOT validate chains, residues, or geometry; for
    that, run ``validate_design_spec()`` (which calls ``boltzgen check``).

    Returns:
        Dict with keys ``valid`` (bool), ``message`` (str), ``path`` (str), and
        on success ``size_bytes`` (int) and ``format`` (str).
    """
    p = Path(path)
    if not p.exists():
        return {'valid': False, 'message': f'Missing target file: {path}', 'path': path}
    suffix = p.suffix.lower()
    if suffix not in TARGET_STRUCTURE_EXTENSIONS:
        return {'valid': False, 'message': f'Unsupported target extension: {suffix}', 'path': path}
    text = p.read_text(encoding='utf-8', errors='ignore')[:10000]
    looks_structural = ('ATOM' in text) or ('_atom_site.' in text) or text.lstrip().startswith('data_')
    if not looks_structural:
        return {'valid': False, 'message': 'File does not look like a PDB or mmCIF structure', 'path': path}
    return {
        'valid': True,
        'message': 'Target structure looks valid',
        'path': str(p),
...
def write_checkpoint(data: Dict[str, Any], path: str = None) -> str:
    """Persist a JSON checkpoint of intermediate state during long runs.

    Discovery's job orchestrator surfaces ``/output`` even if the container
    crashes mid-pipeline, so writing periodic checkpoints (after design,
    after folding, etc.) gives the user something to inspect on failure.
    Values are passed through ``_json_safe`` so numpy/pandas/Path values are
    handled automatically.

    Args:
        data: Arbitrary state dictionary (numpy/pandas values are OK).
        path: Output path. Defaults to ``<OUTPUT_DIR>/checkpoint.json``.

    Returns:
        Absolute path of the written checkpoint.
    """
    if path is None:
