...
class TestSetup:
    ...
    def test_save_final_results_nested_numpy_serialization(self, temp_dir):
        """Recursive _json_safe handles numpy values nested inside lists/dicts."""
        import numpy as np
        utils.OUTPUT_DIR = temp_dir
        utils.save_final_results(
            results={
                'top_designs': [
                    {'id': 'd0', 'plddt': np.float64(0.87), 'rmsd': np.float32(1.2)},
                    {'id': 'd1', 'plddt': np.float64(0.74), 'rmsd': np.float32(2.1)},
                ],
                'matrix': np.array([[1, 2], [3, 4]]),
            },
            status='completed',
        )
        with open(os.path.join(temp_dir, 'final_results.json')) as f:
            data = json.load(f)
        assert data['summary']['top_designs'][0]['id'] == 'd0'
        assert abs(data['summary']['top_designs'][0]['plddt'] - 0.87) < 1e-6
        assert data['summary']['matrix'] == [[1, 2], [3, 4]]
...
class TestInputValidation:
    """Tests for list_target_files / validate_target_file."""

    def test_list_target_files_finds_pdb_and_cif(self, temp_dir):
        for name in ('a.pdb', 'b.cif', 'c.mmcif', 'ignore.txt'):
            with open(os.path.join(temp_dir, name), 'w') as f:
                f.write('ATOM' if name.endswith('.pdb') else 'data_x')
        found = utils.list_target_files(temp_dir)
        names = [os.path.basename(p) for p in found]
        assert names == ['a.pdb', 'b.cif', 'c.mmcif']

    def test_list_target_files_empty(self, temp_dir):
        assert utils.list_target_files(temp_dir) == []

    def test_validate_target_file_missing(self):
        result = utils.validate_target_file('/nope/missing.cif')
        assert result['valid'] is False
        assert 'Missing' in result['message']

    def test_validate_target_file_wrong_extension(self, temp_dir):
        path = os.path.join(temp_dir, 'notes.txt')
        with open(path, 'w') as f:
            f.write('ATOM 1')
        assert utils.validate_target_file(path)['valid'] is False

    def test_validate_target_file_pdb_ok(self, temp_dir):
        path = os.path.join(temp_dir, 'x.pdb')
        with open(path, 'w') as f:
            f.write('HEADER ...\nATOM      1  N   ALA A   1\n')
        result = utils.validate_target_file(path)
        assert result['valid'] is True
        assert result['format'] == 'pdb'
        assert result['size_bytes'] > 0

    def test_validate_target_file_cif_ok(self, temp_dir):
        path = os.path.join(temp_dir, 'x.cif')
        with open(path, 'w') as f:
            f.write('data_test\n_atom_site.id 1\n')
        assert utils.validate_target_file(path)['valid'] is True

    def test_validate_target_file_not_structural(self, temp_dir):
        path = os.path.join(temp_dir, 'x.cif')
        with open(path, 'w') as f:
            f.write('this is just some text without markers\n')
        assert utils.validate_target_file(path)['valid'] is False
...
class TestCheckpoint:
    ...
    def test_write_checkpoint_default_path(self, temp_dir):
        utils.OUTPUT_DIR = temp_dir
        path = utils.write_checkpoint({'step': 'design', 'count': 5})
    ...
    def test_write_checkpoint_handles_numpy(self, temp_dir):
        import numpy as np
        utils.OUTPUT_DIR = temp_dir
        path = utils.write_checkpoint({
            'plddts': np.array([0.7, 0.8, 0.9]),
            'best': np.float64(0.92),
        })
        with open(path) as f:
            data = json.load(f)
        assert data['plddts'] == [0.7, 0.8, 0.9]
        assert abs(data['best'] - 0.92) < 1e-6

    def test_write_checkpoint_explicit_path(self, temp_dir):
        target = os.path.join(temp_dir, 'sub', 'cp.json')
        path = utils.write_checkpoint({'k': 1}, path=target)
        assert os.path.exists(path) and path.endswith('cp.json')
