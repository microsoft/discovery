...
try {
  if (
    ...
    throw new Error(
      'The execution facade is not running in the attested confirmed-run role.',
    )
  }
  const verified =
  ...
  if (
    ...
    throw new Error(
      'The attested hidden-flow runtime does not expose the reviewed execution entry points.',
    )
  }
  const result =
    ...
    `${JSON.stringify({
      ...
      error: {
        ...
        classification: cancelled
          ...
          : 'package-attestation',
        detail: cancelled
          ? 'The portable hidden-flow run was cancelled during bounded package verification.'
