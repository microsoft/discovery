...
This agent operates as a `kind: prompt` agent within Discovery Studio.
    ...
    User Input → PDB Insights (LLM) → PDB Insights Tool (Container) → Results

- **Model:** Configured via the `{{model}}` parameter at deploy time
- **Tool:** PDB Insights container for comprehensive protein structure analysis via RCSB PDB API
...
|---|---|---|
| `pdbInsights` | `tools/PDBInsights/` | PDBInsights is a tool to search, download, prepare, and evaluate PDB structures for protein targets using RCSB APIs, Biopython, and PDBFixer. |
