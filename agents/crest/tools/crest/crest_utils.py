...
# ============================================================================
# xTB PROPERTY ACCESSORS
# ============================================================================
# Structured output wrappers that eliminate fragile
# regex parsing of xTB stdout.
#
# Usage:
#   props = get_xtb_properties('mol.xyz', method='gfn2')
#   print(props['dipole_debye'], props['homo_lumo_gap_eV'])
#
# Or use convenience wrappers:
#   dipole = get_dipole('mol.xyz')
#   gap    = get_homo_lumo_gap('mol.xyz')
# ============================================================================


def get_xtb_properties(coord_file: str, method: str = 'gfn2',
                       charge: int = 0, uhf: int = 0,
                       solvent: str = None) -> dict:
    """Run xTB single-point and extract all electronic properties.

    Args:
        coord_file: Path to XYZ or Turbomole coord file.
        method: GFN method ('gfn2', 'gfn1', 'gfnff').
        charge: Molecular charge.
        uhf: Number of unpaired electrons.
        solvent: ALPB solvent name (e.g., 'h2o', 'dmso') or None.

    Returns:
        dict with keys:
            total_energy_Eh (float): Total energy in Hartree
            homo_energy_eV (float): HOMO energy in eV
            lumo_energy_eV (float): LUMO energy in eV
            homo_lumo_gap_eV (float): HOMO-LUMO gap in eV
            dipole_debye (float): Total dipole moment in Debye
            polarizability_A3 (float | None): Isotropic polarizability in A^3
                                              (None if --alpha parsing fails)
    ...
    args = [f'--{method}', '--chrg', str(charge), '--uhf', str(uhf)]
    if solvent:
        args.extend(['--alpb', solvent])

    result = run_xtb(coord_file, args)
    if result.returncode != 0:
        raise RuntimeError(
    ...
    # Total energy
    m = re.search(r'TOTAL ENERGY\s+([-\d.]+)\s+Eh', stdout)
    props['total_energy_Eh'] = float(m.group(1)) if m else None

    # HOMO-LUMO gap
    m = re.search(r'HOMO-LUMO GAP\s+([-\d.]+)\s+eV', stdout)
    props['homo_lumo_gap_eV'] = float(m.group(1)) if m else None

    # Individual orbital energies
    m = re.search(r'\(HOMO\)\s+([-\d.]+)\s', stdout)
    props['homo_energy_eV'] = float(m.group(1)) if m else None
    m = re.search(r'\(LUMO\)\s+([-\d.]+)\s', stdout)
    props['lumo_energy_eV'] = float(m.group(1)) if m else None

    # Dipole moment - parse the "full:" line for total (4th value).
    # The line has x, y, z, total. We use abs() to guard against
    # the regex accidentally capturing a negative component.
    m = re.search(r'full:\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+([\d.]+)', stdout)
    if m:
        props['dipole_debye'] = float(m.group(4))
    else:
        props['dipole_debye'] = None

    # Polarizability - run --alpha separately (can fail on some versions)
    props['polarizability_A3'] = None
    try:
        alpha_args = args + ['--alpha']
        alpha_result = run_xtb(coord_file, alpha_args)
        if alpha_result.returncode == 0:
            alpha_out = alpha_result.stdout or ''
            # Try "Mol. alpha" line first
            m = re.search(r'Mol\.\s+[Cc]\d?\s*alpha.*?([\d.]+)', alpha_out)
            if m:
                props['polarizability_A3'] = float(m.group(1))
            else:
                # Fallback: "alpha(0)" line
                m = re.search(r'alpha\(0\)\s*[=:]\s*([\d.]+)', alpha_out)
                if m:
                    props['polarizability_A3'] = float(m.group(1))
    except Exception:
        pass  # polarizability is best-effort

    return props


def get_dipole(coord_file: str, method: str = 'gfn2',
               charge: int = 0, solvent: str = None) -> float:
    """Get total dipole moment in Debye from xTB single-point.

    Convenience wrapper around get_xtb_properties().

    Raises:
        ValueError: If dipole could not be parsed.
    """
    props = get_xtb_properties(coord_file, method=method,
                               charge=charge, solvent=solvent)
    if props['dipole_debye'] is None:
        raise ValueError("Could not parse dipole moment from xTB output")
    return props['dipole_debye']


def get_homo_lumo_gap(coord_file: str, method: str = 'gfn2',
                      charge: int = 0, solvent: str = None) -> float:
    """Get HOMO-LUMO gap in eV from xTB single-point.

    Convenience wrapper around get_xtb_properties().

    Raises:
        ValueError: If gap could not be parsed.
    """
    props = get_xtb_properties(coord_file, method=method,
                               charge=charge, solvent=solvent)
    if props['homo_lumo_gap_eV'] is None:
        raise ValueError("Could not parse HOMO-LUMO gap from xTB output")
    return props['homo_lumo_gap_eV']


def get_polarizability(coord_file: str, method: str = 'gfn2',
                       charge: int = 0, solvent: str = None):
    """Get isotropic polarizability in Angstrom^3 from xTB --alpha.

    Returns None (not raises) if xTB --alpha parsing fails, since
    polarizability extraction is known to be unreliable across xTB versions.
    Callers should fall back to RDKit MolMR * 0.3964 if None is returned.
    """
    props = get_xtb_properties(coord_file, method=method,
                               charge=charge, solvent=solvent)
    return props['polarizability_A3']


def get_total_energy(coord_file: str, method: str = 'gfn2',
                     charge: int = 0, solvent: str = None) -> float:
    """Get total energy in Hartree from xTB single-point.

    Convenience wrapper around get_xtb_properties().

    Raises:
        ValueError: If energy could not be parsed.
    """
    props = get_xtb_properties(coord_file, method=method,
                               charge=charge, solvent=solvent)
    if props['total_energy_Eh'] is None:
        raise ValueError("Could not parse total energy from xTB output")
    return props['total_energy_Eh']
