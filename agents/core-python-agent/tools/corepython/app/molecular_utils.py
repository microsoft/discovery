...
def calculate_drug_likeness(smiles: str, method="QED") -> dict:
    ...
    try:
        ...
        if method == "Ghose" or method == "ALL":
            ...
            results["Ghose"] = f"Violations: {ghose_violations}"
        ...
        if method == "Veber" or method == "ALL":
            ...
            results["Veber"] = f"Violations: {veber_violations}"
        ...
        if method == "Egan" or method == "ALL":
            ...
            results["Egan"] = f"Violations: {egan_violations}"
...
def optimize_structure(smiles: str, max_iterations: int = 200, convergence_threshold: float = 1e-4,
                       embedding_method: str = "ETKDG", forcefield: str = "UFF") -> str:
    try:
        # Prepare molecule with hydrogens and embed 3D coordinates using ETKDGv3
        mol = prepare_molecule(smiles, add_hs=True, embed=True)
        mol = Chem.AddHs(mol)
        # Choose embedding parameters based on the specified method
        if embedding_method.lower() == "etkdg":
            params = AllChem.ETKDG()
        elif embedding_method.lower() == "etkdgv3":
            params = AllChem.ETKDGv3()
        else:
            # Fallback to classic ETKDG if method is unrecognized
            params = AllChem.ETKDG()
        params.numThreads = 0  # Use all available threads in embedding
        
        if AllChem.EmbedMolecule(mol, params) != 0:
            raise RuntimeError("Embedding failed")
              # Optimize geometry based on forcefield option
        if forcefield.upper() == "MMFF94":
            try:
                ...
                try:
                    ...
                    logging.warning("UFF optimization succeeded as fallback")
                except Exception as uff_e:
                    logging.warning(f"UFF fallback optimization also failed: {str(uff_e)}")
                    logging.warning("Proceeding with unoptimized structure.")
            ...
            try:
                ...
                try:
                    ...
                    if properties is not None:
                        AllChem.MMFFOptimizeMolecule(mol, maxIters=max_iterations)
                        logging.warning("MMFF94 optimization succeeded as fallback")
                    else:
                    ...
                    logging.warning("Proceeding with unoptimized structure.")
        elif forcefield.upper() == "NONE":
            # Skip optimization
            pass
        else:
            # Default to UFF if unrecognized forcefield
            try:
                AllChem.UFFOptimizeMolecule(mol, maxIters=max_iterations)
            except Exception as e:
                logging.warning(f"UFF optimization failed for SMILES {smiles}: {str(e)}")
...
def calculate_partial_charges(smiles: str, method: str = "Gasteiger", iterations: int = 10, output_format: str = "detailed"):
    ...
    try:
        ...
        if method.lower() == "gasteiger":
            Chem.rdPartialCharges.ComputeGasteigerCharges(mol, maxIter=iterations)
        else:
