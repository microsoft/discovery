# bookshelfResearcher -- KB-Grounded Research Agent (LLM-Only)

## Overview

**bookshelfResearcher** is an LLM-only agent (no compute tools) for the Microsoft Discovery platform.
It conducts literature reviews and research analyses in scientific, engineering, and medical fields,
grounded in the user's attached Knowledge Bases / Bookshelves, providing comprehensive answers with:

- **KB-grounded citations** -- numbered inline references [1], [2] pointing to KB documents (title/path/section/page)
...
catalog_publish_agent(agent_key="bookshelfResearcher", ...)
```
...
- **No live internet access** -- does not browse the web
- **KB coverage limits** -- findings are limited to what's present in the attached KB; recommend additions when missing
- **Citation precision depends on KB metadata** -- cite the KB's title/path/section/page when available
...
This agent operates as a `kind: prompt` LLM-only agent within Discovery Studio — no containerized tools are required.
    ...
    User Input → Bookshelf Researcher (LLM) → Knowledge Base Retrieval → Structured Research Output

- **Model:** Configured via the `{{model}}` parameter at deploy time
- **External dependencies:** User-attached Knowledge Bases / Bookshelves

## Prerequisites

- Azure subscription with Contributor role
- Azure AI Foundry project with a model deployment (e.g. GPT-4o)
- One or more Knowledge Bases / Bookshelves attached in Discovery Studio
