...
This agent operates as a `kind: prompt` agent within Discovery Studio.
    ...
    User Input → Molecular Groups (LLM) → Molecular Groups Tool (Container) → Results

- **Model:** Configured via the `{{model}}` parameter at deploy time
- **Tool:** Molecular Groups container for functional group identification and hazardous group safety screening
...
|---|---|---|
| `molecularGroups` | `tools/molecularGroups/` | A specialized tool for identifying functional groups in molecular structures. Provides both general functional group identification (carbonyls, ami... |
