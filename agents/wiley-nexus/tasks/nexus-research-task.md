...
Search Wiley Nexus from multiple distinct angles and build the evidence base that the later stages of this review will work from. Search and evidence gathering only: do not download full texts and do not write the final answer.

Produce two files.

File 1 - evidence_passages.md (markdown). Open with a "## Search coverage" section containing one bullet per search angle, giving the query used, what it supported, and any facet that returned little or nothing. Then a "## Publications" section with one "### [n] Title" entry per unique publication, deduplicated by DOI, each carrying these labelled fields: DOI, Authors, Journal and year, Full Text Available (Yes or No), Wiley Online Library link. Under each entry, include that publication's passages quoted verbatim as blockquotes.

File 2 - fulltext_shortlist.json (JSON), of the form {"shortlist":[{"doi":"...","title":"...","rationale":"...","full_text_token":"..."}]}, listing the 3 to 5 publications most worth reading in full. Only shortlist papers whose search hit carried a full_text_token, and copy each token in verbatim.
...
3. Each analysis reports what its own paper found rather than summarizing the topic generally.
4. Each file records the methods and the limitations of the paper it covers.
```
...
3. Reference entries include DOIs and link to Wiley Online Library.
