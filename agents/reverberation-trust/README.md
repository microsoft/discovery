...
| **Evidence / tool** | Five estimators (ICCF, DCF, von Neumann, DRW, JAVELIN) + a witness/trust layer (sampling-window fact-arbiter, red-noise & aliasing gates, cross-band/line/instrument Look witnesses, the X-ray-timing Channel cascade). |
...
estimate with independent classes (ICCF, DCF, von Neumann, DRW forward-model, JAVELIN) → read the
